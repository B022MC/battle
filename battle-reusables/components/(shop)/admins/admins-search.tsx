...
type AdminsSearchParams = z.infer<typeof searchFormSchema>;

type AdminsSearchProps = {
  submitButtonProps?: ButtonProps & { loading?: boolean };
  onSubmit?: (data: API.ShopsAdminsListParams) => void;
};

export const AdminsSearch = ({ submitButtonProps, onSubmit }: AdminsSearchProps) => {
  const {
    control,
    handleSubmit,
    formState: { isValid },
  } = useForm<AdminsSearchParams>({
    resolver: zodResolver(searchFormSchema),
    defaultValues: { houseGid: '' },
    mode: 'all',
  });

  const handleParams = (params: AdminsSearchParams) => {
