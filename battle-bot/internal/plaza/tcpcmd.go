...
// CmdUserLookOn 旁观请求（可能是进入桌台的正确方式）
func CmdUserLookOn(tableID int) *game.Packer {
	packer := &game.Packer{}
	packer.SetCmd(consts.MDM_GR_USER, consts.SUB_GR_USER_LOOKON)
	packer.PushWord(uint16(tableID))
	packer.PaddingTo(2)
	return packer
}

func CmdUserSitDown(tableID, chairID int, pwd string) *game.Packer {
	packer := &game.Packer{}
	packer.SetCmd(consts.MDM_GR_USER, consts.SUB_GR_USER_SITDOWN)
	packer.PushWord(uint16(tableID))
...
// CmdUserSitDownWithPassword 带密码的坐下命令（如果需要）
func CmdUserSitDownWithPassword(tableID, chairID int, pwd string) *game.Packer {
	packer := &game.Packer{}
	packer.SetCmd(consts.MDM_GR_USER, consts.SUB_GR_USER_SITDOWN)
	packer.PushWord(uint16(tableID))
	packer.PushWord(uint16(chairID))
...
func CmdUserReady() *game.Packer {
	packer := &game.Packer{}
	packer.SetCmd(consts.MDM_GF_FRAME, consts.SUB_GF_USER_READY)
	packer.PaddingTo(0)
	return packer
}
