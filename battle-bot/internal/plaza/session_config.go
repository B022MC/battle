...
import (
	"battle-bot/internal/plaza/consts"
...
func (that *Session) _87handlePacket(data []byte) {
	...
	case consts.MDM_GA_BATTLE_SERVICE: // 1
		...
		case consts.SUB_GA_TABLE_ITEM: // 200 - 桌子信息（可能是坐下响应）
			logger.Info("收到桌子信息（可能是坐下响应）")
			rawHex := hexHead(packer.Data(), len(packer.Data()))
			logger.Infof("桌子信息数据: DataLen=%d, Data=%s", len(packer.Data()), rawHex)
		case 204: // SUB_GA_TABLE_PARAMLIST - 桌子参数
			logger.Info("收到桌子参数列表")
			rawHex := hexHead(packer.Data(), min(len(packer.Data()), 256))
			logger.Infof("桌子参数数据: DataLen=%d, Data=%s", len(packer.Data()), rawHex)
			// 注意：这个可能是查询桌台后的响应，不是坐下的响应
		...
		default:
			logger.Infof("未处理的BattleService数据包: SubCmdID=%d, DataLen=%d", packer.Head.Cmd.SubCmdID, len(packer.Data()))
		}

	case consts.MDM_GF_FRAME: // 100
		logger.Infof("收到框架命令: SubCmdID=%d, DataLen=%d", packer.Head.Cmd.SubCmdID, len(packer.Data()))
		switch packer.Head.Cmd.SubCmdID {
		case consts.SUB_GF_GAME_STATUS: // 100
			logger.Info("收到游戏状态")
		case consts.SUB_GF_GAME_SCENE: // 101
			logger.Info("收到游戏场景")
			rawHex := hexHead(packer.Data(), min(len(packer.Data()), 256))
			logger.Infof("游戏场景数据: %s", rawHex)
		default:
			rawHex := hexHead(packer.Data(), min(len(packer.Data()), 64))
			logger.Infof("未处理的框架命令: Data=%s", rawHex)
		}

	case consts.MDM_GF_GAME: // 200
		logger.Infof("收到游戏命令: SubCmdID=%d, DataLen=%d", packer.Head.Cmd.SubCmdID, len(packer.Data()))
		rawHex := hexHead(packer.Data(), min(len(packer.Data()), 256))
		logger.Infof("游戏数据: %s", rawHex)
	...
	case consts.MDM_GA_GROUP_SERVICE: // 3 (也是 MDM_GR_USER，值相同)
		logger.Infof("收到 MainCmdID=3, SubCmdID=%d", packer.Head.Cmd.SubCmdID)
		switch packer.Head.Cmd.SubCmdID {
		// MDM_GR_USER 相关命令
		case consts.SUB_GR_USER_SITDOWN: // 3 - 坐下响应
			logger.Info("✅ 收到坐下响应！")
			rawHex := hexHead(packer.Data(), len(packer.Data()))
			logger.Infof("坐下响应数据: DataLen=%d, Data=%s", len(packer.Data()), rawHex)
		case consts.SUB_GR_USER_STANDUP: // 4 - 站起响应
			logger.Info("收到站起响应")
		// MDM_GA_GROUP_SERVICE 相关命令
		case consts.SUB_GA_GROUP_MEMBER: // 102
			that._87waitingForCmdResponse.Store(false)
		...
		case 200: // SUB_GA_ENTER_SUCCESS
			logger.Info("进入房间成功")
			that._87waitingForCmdResponse.Store(false)
		case 201: // SUB_GA_ENTER_FAILURE or SUB_GA_OPERATE_FAILURE
			that._87waitingForCmdResponse.Store(false)
			msg := ParseSystemMessage(packer.Data())

			// 打印原始数据包以调试
			rawHex := hexHead(packer.Data(), len(packer.Data()))
			logger.Errorf("❌ GROUP_SERVICE操作失败(201): %s (DataLen=%d)", msg.Text, len(packer.Data()))
			logger.Errorf("原始数据: %s", rawHex)
			...
			// 3. 账号未加入该房间
			if msg.Text == "" {
				logger.Errorf("❌ 无法进入茶馆！HouseGID=%d, UserID=%d", that.houseGID, that.userID)
				logger.Errorf("可能的原因：")
				logger.Errorf("  1. 账号未加入该茶馆")
				logger.Errorf("  2. 茶馆ID错误")
				logger.Errorf("  3. 账号被该茶馆封禁")
			}
	...
	default:
		rawHex := hexHead(packer.Data(), 128) // 显示前128字节
		logger.Infof("未处理的87数据包: MainCmdID=%d, SubCmdID=%d, DataLen=%d",
			packer.Head.Cmd.MainCmdID, packer.Head.Cmd.SubCmdID, len(packer.Data()))
		if len(packer.Data()) > 0 {
			logger.Infof("数据内容: %s", rawHex)
		}
	}
}
...
// SendCommand 发送通用命令
func (that *Session) SendCommand(packer *game.Packer) error {
	if !that._87connReady.Load() {
		return fmt.Errorf("87服务器连接未就绪")
	}

	logger.Infof("发送命令: MainCmdID=%d, SubCmdID=%d, DataLen=%d",
		packer.Head.Cmd.MainCmdID, packer.Head.Cmd.SubCmdID, len(packer.Data()))

	data := that._87encoder.Encrypt(packer)
	if _, err := that._87connection.Write(data); err != nil {
